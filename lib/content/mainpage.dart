// ignore_for_file: prefer_final_fields, unnecessary_string_interpolations, prefer_const_constructors, prefer_interpolation_to_compose_strings, use_build_context_synchronously, prefer_const_constructors_in_immutables, no_leading_underscores_for_local_identifiers
import 'package:barcodeapp/content/button/buildbutton01.dart';
import 'package:barcodeapp/content/button/buildbutton02.dart';
...
class MainPage extends StatefulWidget {
  ...
  @override
  SMainPageState createState() => SMainPageState();
}

class SMainPageState extends State<MainPage> {
  ...
  initState() {
    ...
    dbs.loadData = false;
    getCheckNoV();
    dbs.checkNo = box.read("UserID");
    userIds = box.read("UserID");
    _user.text = dbs.users;
    _checkNo.text = dbs.checkNo;
    //dbs.checkNo = "S10001SS";
  }
  ...
  Future<void> _showMyDialogUpload(String _ckNo) async {
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title:
              Text('Back to Main Page?', style: TextStyle(color: Colors.black)),
          content: SingleChildScrollView(
            child: Column(
              children: const <Widget>[
                Text(
                  'ต้องการกลับไปหน้าแรก หรือไม่?',
